...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Company subscription limits supersede per-ad package limits.
    const maxChars = s.company?.plan?.maxAdChars ?? s.package?.maxChars ?? 1200;
    const limitSource = s.company ? `company:${s.company.plan?.slug}` : (s.package ? `package:${s.package.name}` : "default");
    console.log(`[plan] submission=${id} chars=${text.length}/${maxChars} source=${limitSource}`);

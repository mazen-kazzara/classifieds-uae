...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!cat || !cat.isActive) return NextResponse.json({ ok: false, error: "CATEGORY_NOT_FOUND" }, { status: 404 });
  if (cat.parentId) return NextResponse.json({ ok: false, error: "SUBCATEGORY_NOT_ALLOWED", message: "Only main categories are allowed." }, { status: 400 });

    ...
    <div style={{ minHeight: "100vh", backgroundColor: "var(--bg)" }}>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <Link href="/new" className="btn-primary" style={{ height: 36, padding: "0 1rem", fontSize: "0.875rem", textDecoration: "none" }}>{locale === "ar" ? "+ نشر إعلان" : "+ Post Ad"}</Link>
        </div>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center gap-2 mt-8">
            {page > 1 && <Link href={`/category/${slug}?type=${contentType}&page=${page-1}`} className="btn-secondary" style={{ textDecoration: "none" }}>{locale === "ar" ? "التالي →" : "← Previous"}</Link>}
            ...
            {page < totalPages && <Link href={`/category/${slug}?type=${contentType}&page=${page+1}`} className="btn-secondary" style={{ textDecoration: "none" }}>{locale === "ar" ? "→ السابق" : "Next →"}</Link>}
          </div>
        )}
      </main>
    <Footer />

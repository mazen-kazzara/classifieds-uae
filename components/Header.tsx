    ...
    <header style={{ backgroundColor: "var(--surface)", borderBottom: "1.5px solid var(--border)" }} className="sticky top-0 z-50 shadow-card">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between gap-4 h-[60px]">
          ...
          {/* Search — desktop */}
          <form action="/search" method="GET" className="flex-1 max-w-xl hidden sm:flex">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {status === "authenticated" ? (
              <>
                <Link href="/my-ads" className="btn-ghost" style={{ height: 36, padding: "0 0.75rem", fontSize: "0.8125rem", fontWeight: 600, color: "var(--primary)" }}>
                  {t("myAds")}
                </Link>
                <button onClick={() => signOut({ callbackUrl: "/" })} className="btn-ghost" style={{ height: 36, padding: "0 0.75rem", fontSize: "0.8125rem" }}>
                  {t("signOut")}
                </button>
              </>
            ) : (
              <Link href="/login" className="btn-ghost" style={{ height: 36, padding: "0 0.75rem", fontSize: "0.8125rem", fontWeight: 600 }}>
                {t("signIn")}
              </Link>
            )}
            <Link href="/new" className="btn-primary gap-1" style={{ height: 36, padding: "0 0.875rem", fontSize: "0.8125rem" }}>
